...
export default function WeatherDashboard({ initialCitiesWithWeather }: WeatherDashboardProps) {
  ...
  useEffect(() => {
    const hasFailedFetches = initialCitiesWithWeather.some(c => c.weather === null);
    if (hasFailedFetches) {
      toast({
        variant: "destructive",
        title: "Failed to load some weather data",
        description: "There was a problem fetching weather for one or more cities.",
      })
    }
    setIsLoading(false);
  ...
  const { pinnedCities, unpinnedCities } = useMemo(() => {
    const filtered = initialCitiesWithWeather.filter(c => 
    ...
    <div className="space-y-8">
      ...
      {isLoading ? (
         <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
           {Array.from({ length: 12 }).map((_, i) => <WeatherCardSkeleton key={i} />)}
         </div>
        ...
        <div className='space-y-8'>
              ...
              <section>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
                  {pinnedCities.map(({ city, weather }) => (
                    weather ? <WeatherCard key={city.id} cityWithWeather={{ city, weather }} unit={unit} isPinned={true} onPinToggle={togglePin} onViewForecast={handleViewForecast} /> : null
                  ))}
                </div>
               ...
               <section>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
                    {unpinnedCities.map(({ city, weather }) => (
                      weather ? <WeatherCard key={city.id} cityWithWeather={{ city, weather }} unit={unit} isPinned={false} onPinToggle={togglePin} onViewForecast={handleViewForecast} /> : null
                    ))}
                  </div>
               </section>
            )}
            ...
            {pinnedCities.length === 0 && unpinnedCities.length === 0 && (
                <div className="text-center py-16">
                    <p className="text-muted-foreground">No cities found. {searchTerm && "Try a different search."}</p>
                </div>
            )}
        </div>
        ) : (
